	...
	return (
		<div className='countdown'>
			...
			{project == 'Rully_Sabil_Mobile' && (
				<>
					<div className='justify-center items-center text-[12.5px] grid grid-cols-2 px-3'>
						{Object.entries(timeLeft).map((el, i) => {
							const label = el[0];
							const value = el[1];
							const end = (Object.entries(timeLeft).length - 1);
							return (
								<div key={label} className=" justify-center items-center mt-5">
									<div className="text-black justify-center items-center text-center">
										<div className={'text-[40px] font-Suranna tracking-normal w-full leading-[75px]'}>
											{value || 0}
										</div>
										<div className={"text-[11px] font-inter capitalize mt-[-15px] tracking-wider"}>
											{label}
										</div>
									</div>
								</div>
