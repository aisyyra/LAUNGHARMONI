	...
	return (
		<div className='countdown'>
			...
			{project == 'Rully_Sabil_Mobile' && (
				<>
					<div className='justify-center items-center text-[12.5px] grid grid-rows-4 pt-[7px]'>
						{Object.entries(timeLeft).map((el, i) => {
							const label = el[0];
							const value = el[1];
							const end = (Object.entries(timeLeft).length - 1);
							return (
								<div key={label} className="flex">
									<div className="flex-1 text-black justify-center items-center text-center">
										...
										<div className={Playfair_Display.className + " text-[8.5px] capitalize italic mt-[-15px] tracking-wider"}>
											{label}
										</div>
									</div>
								</div>
